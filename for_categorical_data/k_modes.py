#!/usr/bin/env python
'''Noted by Tai Dinh
File này dùng để chạy giải thuật k-modes
'''
import sys
import getopt
import numpy as np
...
def do_kr(x, y, nclusters=4, verbose=1, n_init=10):
    kr = kmodes.KModes(n_clusters=nclusters, max_iter=1, init='Huang', n_init=n_init, verbose=verbose)
    kr.fit_predict(x)
...
def run(argv):
    ...
    ofile = "output/kmodes_breast.csv"
