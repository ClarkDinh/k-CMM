...
def kcentermod(X, n_clusters, init, n_init, max_iter, verbose, use_global_attr_count, beta):
    ...
    for init_no in range(n_init):
        ...
        while itr <= max_iter and not converged:
            ...
            labels, ncost = _labels_cost(X, centroids, global_attr_freq, w, beta)
...
class KCenterMod(object):
    ...
    def __init__(self, n_clusters = 8, init = 'random', n_init = 10, max_iter = 100,
        verbose = 0, use_global_attr_count = 1, beta = 8):
    ...
    def fit(self, X, **kwargs):
        ...
        self.cluster_centroids_, self.labels_, self.cost_ = \
            kcentermod(X, self.n_clusters, self.init,
                self.n_init, self.max_iter, self.verbose, self.use_global_attr_count, self.beta)
        return self
