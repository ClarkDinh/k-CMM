...
def init_cao_2(X, n_clusters, dissim):
...
def init_cao(X, n_clusters, global_attr_freq):
    """Initialize centroids according to method by Cao et al. [2009].
    ...
    if n_clusters > 1:
        ...
        for ik in range(1, n_clusters):
            ...
            for ikk in range(ik):
                ...
                dd[ikk] = ITBD(X, centroids[ikk], global_attr_freq) * dens
            centroids[ik] = X[np.argmax(np.min(dd, axis=0))]
...
def _k_modes_iter(X, centroids, cl_attr_freq, membship, dissim, random_state):
    ...
    for ipoint, curpoint in enumerate(X):
        ...
        if not membship[old_clust, :].any():
            from_clust = np.sum(membship,axis=1).argmax()
...
class Init(BaseEstimator, ClusterMixin):
